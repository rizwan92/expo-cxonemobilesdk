...
enum ConnectionBridge {
    ...
    static func getChannelConfiguration(env: String, brandId: Int, channelId: String) async throws
        -> ChannelConfiguration
    {
        NSLog(
            "[ExpoCxonemobilesdk] Connection.getChannelConfiguration env=\(env) brandId=\(brandId) channelId=\(channelId)"
        )
        guard let environment = Environment(rawValue: env.uppercased()) else {
            let err = NSError(
                domain: "ExpoCxonemobilesdk",
