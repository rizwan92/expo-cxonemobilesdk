...
> **Important:** the version provided here must match the `spm_dependency` declared in `ios/ExpoCxonemobilesdk.podspec` (currently `3.1.1`). Supplying a lower version causes Xcode to fail resolving package dependencies because it sees conflicting ranges for `nice-cxone-mobile-sdk-ios`.
...
```jsonc
{
  "expo": {
    "plugins": [
      ["expo-cxonemobilesdk/plugin-spm", { "version": "3.1.1", "...": "..." }],
