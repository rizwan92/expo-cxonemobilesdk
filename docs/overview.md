...
- **Native modules** live in `ios/ExpoCxonemobilesdkModule.swift` and `android/src/main/java/expo/modules/cxonemobilesdk/**`. They expose the shared function set (`prepare`, `connect`, `threads*`, customer helpers, analytics, etc.), emit events, and log everything via `NSLog("[ExpoCxonemobilesdk] …")` / Android `Log`.
...
The JS helpers simply await the initial action (e.g., `await Connection.prepare()` then `await Connection.connect()`) and defer long‑running state changes to those events. When adding new flows, prefer emitting events from native code instead of building polling helpers in TypeScript.
