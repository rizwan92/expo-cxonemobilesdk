...
> **Version mismatch warning:** the podspec currently declares `CXoneChatSDK` via `spm_dependency` at `3.0.1`. Use the same `version` in your plugin config (as shown above) or Xcode will fail with “Failed to resolve dependencies … depends on 'nice-cxone-mobile-sdk-ios' 1.x and 3.x”.
